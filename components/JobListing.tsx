...
const JobListing = (job: Job) => {
  ...
  return (
    <View className='w-full p-4 rounded-full'>
      ...
      <Text className='font-quicksand-medium text-lg'>{job.company} {'\u00B7'} {job.location}</Text>
      <Text className='font-quicksand-semibold text-sm'>${job.salary}</Text>
      <ScrollView className='flex-row flex-wrap gap-2 mt-2' horizontal showsHorizontalScrollIndicator={false}>
        {job.tags?.map((tag, index) => (
          <Text key={index} className='bg-gray-200 rounded-full px-3 py-1 text-sm mx-1'>{tag}</Text>
        ))}
      </ScrollView>
    </View>
  )

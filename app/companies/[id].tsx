...
const CompanyInfo = () => {
  ...
  const { hasValidCompanyInformation, refreshCompanyInformation, isLoadingCompanyInformation, getCompanyInformation } =
    useCompanyStore();
  const { data: jobs, isLoading: isLoadingJobs } = useMostRecentJobsAtCompany(Number(id));
  const [jobCount, setJobCount] = useState(0);

  const company = getCompanyInformation(Number(id));
  const isLoadingCompany = isLoadingCompanyInformation();

  useEffect(() => {
    const validCache = hasValidCompanyInformation(Number(id));
    if (!validCache) {
      refreshCompanyInformation(Number(id));
    }
  }, [id]);
  ...
  useEffect(() => {
    const fetchJobCount = async () => {
      try {
        const response = await fetch(`http://192.168.2.29:8080/api/jobs/companies/${id}/job-count`);
        ...
        <>
          <View className="px-4 pb-4 border-b border-gray-100 mt-4">
            <View className="flex-row items-center gap-3">
              ...
              {company && company.id && userType !== "business" && <FavoriteCompany company={company!} />}
            </View>
          </View>
          <ScrollView
                  ...
                  <View className="flex-row items-center gap-2">
                    ...
                    {company?.numEmployees && (
                      <View className={`${sizeCategory.bg} px-2 py-0.5 rounded-full`}>
                        <Text className={`font-quicksand-bold text-xs ${sizeCategory.color}`}>
                          {sizeCategory.label}
                        </Text>
                      </View>
                    )}

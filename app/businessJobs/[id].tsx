...
const BusinessJobDetails = () => {
  ...
  const { data: job, isLoading } = useJobsForBusiness(
    Number(companyId),
    Number(jobId)
  );
  const {
    data: shortListedCandidates,
    isLoading: loadingShortListedCandidates,
  } = useShortListedCandidatesForJob(Number(jobId));
      ...
      {job && (
        <View className="p-4">
          <View className="flex-row items-start justify-between">
            <View>
              <Text className="text-2xl font-bold w-2/3">{job.title}</Text>
              ...
              <Text className="font-quicksand-semibold text-sm">
                Application window closes on {formatDate(job.appDeadline)}
              </Text>
            </View>
            <View className="gap-2">
              <TouchableOpacity
                onPress={() =>
                  router.push(`/businessJobs/applications/${jobId}`)
                }
              >
              ...
              <TouchableOpacity
                onPress={() =>
                  router.push(
                    `/businessJobs/applications/${jobId}?shortListed=true`
                  )
                }
              >
                <Text className="font-quicksand-semibold text-sm text-black border border-black px-2 py-1 rounded-full">
                  {loadingShortListedCandidates
                    ? "Loading..."
                    : `${shortListedCandidates?.length || 0} Shortlisted`}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View>
            ...
            <Text className="font-quicksand-regular text-base">
              {job.description}
            </Text>
          </View>
          <View className="divider my-4" />

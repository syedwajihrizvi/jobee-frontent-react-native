...
const ApplicantForBusiness = () => {
  ...
  const { data: shortListedCandidates } = useShortListedCandidatesForJob(
  ...
  useEffect(() => {
    if (shortListedCandidates && application) {
      setIsShortListed(
        shortListedCandidates?.includes(application?.id) || false
      );
    }
        ...
        <>
          ...
          <ScrollView className="p-4">
              ...
              {showExperience &&
                userProfile?.experiences.map((exp) => (
                  <View key={exp.id} className="mb-2">
                    ...
                    <Text className="font-quicksand-regular text-base mt-1">
                      {exp.description}
                    </Text>
                  </View>
                ))}
            ...
            {userProfile?.videoIntroUrl && (
              <View className="mt-4">
                <UserVideoIntro
                  videoSource={getS3VideoIntroUrl(userProfile.videoIntroUrl)}
                />
              </View>
            )}
          </ScrollView>

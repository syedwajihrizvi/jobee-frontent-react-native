...
const ApplicantForBusiness = () => {
  ...
  const handleEmailApplicant = () => {
    ...
    const query = [];
    query.push(
      `subject=${encodeURIComponent(`Regarding your application for ${application?.jobTitle} at ${application?.companyName}`)}`
    );
    if (query.length > 0) {
      emailUrl += `?${query.join("&")}`;
    }
  ...
  const handleSMSApplicant = () => {
    ...
    const body =
      "?body=Hello" +
      encodeURIComponent(userProfile?.firstName || "") +
      ` .I saw you application on Jobee for ${application?.jobTitle} at ${application?.companyName}.`;
